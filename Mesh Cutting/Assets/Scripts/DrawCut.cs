...
public class DrawCut : MonoBehaviour
{
    ...
    {
        Vector3 pointInPlane = (pointA.position + pointB.position) / 2;
        
        Vector3 cutPlaneNormal = Vector3.Cross((pointA.position - pointB.position),(pointA.position - cam.transform.position)).normalized;
       ...
       // boxVis.position = pointInPlane;
        ...
        var all = Physics.OverlapBox(pointInPlane, new Vector3(4, 4, 4), orientation);
        ...
        //var all = Physics.RaycastAll(ray);
        {
            foreach (var hit in all)
            {
                ...
                if(filter != null && !filter.CompareTag("Obstacles"))
